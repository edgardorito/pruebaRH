  ...
  $.ajax({
    ...
    error: function(data){
      if(data.status === 422) {
        ...
        var html = '<div class="alert alert-danger" role="danger">  NO. DEBTOR IS DUPLICATE </div>';
        $divErros.html(html)    
      }
      dataTable()
    ...
    success: function(){
      var html = '<div class="alert alert-success" role="success"> ¡SAVE!  </div>';
      $divErros.html(html)  
      $formAddDM[0].reset()
      dataTable()
...
function delete_row(debtorno) {
  $.ajax({
    ...
    var html ='';
    if (deleteSucces) {
      html = '<div class="alert alert-success"> DELETE </div>';
      
    }else{
      html = '<div class="alert alert-danger"> ERROR </ul> </div>';      
    }
